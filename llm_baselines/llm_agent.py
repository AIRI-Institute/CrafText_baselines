    ...
    def act(self, text_observation, instruction):
       ...
       # print(prompt)
        ...
        for action in self.available_actions:
            index = llm_answer.rfind(action)
            if index > latest_index:
                latest_index = index
                latest_action = action

        self.history.append(f"{self.step_counter}: {latest_action}")
...
def main():
    ...
    agent = HuggingfaceAgent(dummy_mode=False)
    instruction = "Set up a furnace for smelting."
    for _ in range(10):
        action_index = agent.act(text_observation, instruction)
        print(f"Chosen action: {agent.available_actions[action_index]}")
